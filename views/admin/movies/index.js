...
exports.find = function(req, res, next){
  ...
  req.app.db.models.Status.pagedFind({
    ...
    if (req.xhr) {
      ...
      results.filters = req.query;
      res.render('admin/movies/index', { data: { results: escape(JSON.stringify(results)) } });
    }
  });
};
...
exports.read = function(req, res, next){
  req.app.db.models.Status.findById(req.params.id).exec(function(err, status) {
    ...
    if (req.xhr) {
      ...
      res.render('admin/movies/details', { data: { record: escape(JSON.stringify(status)) } });
    }
  });
};
...
exports.create = function(req, res, next){
  ...
  workflow.on('validate', function() {
    ...
    if (!req.user.roles.admin.isMemberOf('root')) {
      workflow.outcome.errors.push('You may not create movies.');
      return workflow.emit('response');
    }

    // Параметр pivot задан
    if (!req.body.pivot) {
  ...
  // Проверка совпадения - такое уже есть в базе
...
exports.update = function(req, res, next){
  ...
  workflow.on('validate', function() {
    if (!req.user.roles.admin.isMemberOf('root')) {
      workflow.outcome.errors.push('You may not update movies.');
      return workflow.emit('response');
    }
...
exports.delete = function(req, res, next){
  ...
  workflow.on('validate', function() {
    if (!req.user.roles.admin.isMemberOf('root')) {
      workflow.outcome.errors.push('You may not delete movies.');
      return workflow.emit('response');
    }
